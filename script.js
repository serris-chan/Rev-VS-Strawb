...
const player2 = {
    ...
    direction: 'left', // Sets the initial direction Player 1 is facing
...
// Special function (I just copy-pasted the attack func. Should I just implement it to the attach func? - KyuByt)
function special(attacker, defender) {
    // Checks if the attacker is close enough to the defender to land a hit (I changed values to test - KyuByt)
    if (Math.abs(attacker.x - defender.x) < 60 && Math.abs(attacker.y - defender.y) < 60) {
...
document.addEventListener('keydown', (event) => { // Listens for keyboard input for attacks
    if (event.key === ' ') { // If Spacebar is pressed
        ...
        special(player1, player2); // (I can't code for shit but i can half way read it - KyuByt)
    }
